...
class MainActivity : AppCompatActivity() {
    ...
    private val getBios = registerForActivityResult(ActivityResultContracts.GetContent()) { u -> u?.let { bios = read(it); Toast.makeText(this, "BIOS OK: " + bios?.size, 0).show(); tryStart() } }
    private val getRom = registerForActivityResult(ActivityResultContracts.GetContent()) { u -> u?.let { rom = read(it); Toast.makeText(this, "ROM OK: " + rom?.size, 0).show(); tryStart() } }
